// RSSB — समान पात्रता परीक्षा (स्नातक स्तर) — आधिकारिक परीक्षा स्कीम व पाठ्यक्रम

export interface ExamSchemeSection {
  id: string;
  title: string;
  points: string[];
}

export const EXAM_SCHEME_META = {
  board: "राजस्थान कर्मचारी चयन बोर्ड, जयपुर",
  examName: "समान पात्रता परीक्षा (स्नातक स्तर) - 2026",
  totalQuestions: 150,
  totalMarks: 300,
  duration: "03:00 घंटे",
  notes: [
    "सभी प्रश्न बहुविकल्पी होंगे तथा सभी प्रश्नों के अंक समान होंगे।",
    "किसी प्रश्न के गलत उत्तर के लिए परीक्षार्थी के प्राप्तांकों में से उस प्रश्न के पूर्णांक का एक-तिहाई (1/3) अंक (Negative Marking) काटा जावेगा।",
  ],
};

export const EXAM_SCHEME_SECTIONS: ExamSchemeSection[] = [
  {
    id: "history-freedom",
    title: "भारतीय राष्ट्रीय आन्दोलन पर विशेष बल के साथ राजस्थान और भारत का इतिहास",
    points: [
      "भारतीय इतिहास की महत्त्वपूर्ण घटनाएं — 19वीं एवं 20वीं शताब्दी में सामाजिक एवं धार्मिक सुधार आंदोलन",
      "भारतीय स्वतंत्रता आंदोलन की विभिन्न अवस्थाएं, इनमें देश के विभिन्न क्षेत्रों के योगदानकर्ता एवं उनका योगदान",
      "1857 की क्रान्ति में राजस्थान का योगदान, राजस्थान में जनजाति व किसान आन्दोलन, राजनीतिक जनजागरण एवं प्रजामण्डल आंदोलन",
      "स्वातंत्र्योत्तर राष्ट्र निर्माण — राष्ट्रीय एकीकरण एवं राज्यों का पुनर्गठन, नेहरू युग में सांस्थानिक निर्माण, विज्ञान एवं तकनीकी का विकास",
    ],
  },
  {
    id: "rajasthan-history-culture",
    title: "राजस्थान का इतिहास, कला, सांस्कृतिक, साहित्य, परंपरा और विरासत",
    points: [
      "प्राचीन सभ्यताएं — कालीबंगा, आहड़, गणेश्वर, बालाथल और बैराठ",
      "राजस्थान के इतिहास की महत्वपूर्ण ऐतिहासिक घटनाएं, प्रमुख राजवंश, उनकी प्रशासनिक व राजस्व व्यवस्था, सामाजिक-सांस्कृतिक आयाम",
      "स्थापत्य कला की प्रमुख विशेषताएं — किले एवं स्मारक, कलाएं, चित्रकलाएं और हस्तशिल्प",
      "राजस्थानी साहित्य की महत्वपूर्ण कृतियां, क्षेत्रीय बोलियां",
      "मेले, त्योहार, लोक संगीत एवं लोक नृत्य",
      "राजस्थानी संस्कृति, परम्परा एवं विरासत",
      "राजस्थान के धार्मिक आंदोलन, संत एवं लोक देवता",
      "महत्वपूर्ण ऐतिहासिक स्थल",
      "राजस्थान के प्रमुख व्यक्तित्व",
      "राजस्थान का एकीकरण",
    ],
  },
  {
    id: "geography-india",
    title: "भारत का भूगोल",
    points: [
      "भौतिक स्वरूप — पर्वत, पठार, मरूस्थल एवं मैदान",
      "जलवायु एवं मानसून तंत्र",
      "प्रमुख नदियाँ, बांध, झीलें एवं सागर",
      "वन्य जीव जन्तु एवं अभयारण्य",
      "प्रमुख फसलें — गेहूँ, चावल, कपास, गन्ना, चाय एवं कॉफी",
      "प्रमुख खनिज — लौह अयस्क, मैंगनीज, बॉक्साइट एवं अभ्रक",
      "ऊर्जा संसाधन — परम्परागत एवं गैर-परम्परागत",
      "प्रमुख उद्योग एवं औद्योगिक प्रदेश",
      "राष्ट्रीय राजमार्ग, परिवहन के साधन एवं व्यापार",
    ],
  },
  {
    id: "geography-rajasthan",
    title: "राजस्थान का भूगोल",
    points: [
      "भूगर्भिक संरचना एवं भू-आकृतिक प्रदेश",
      "जलवायु दशाएं, मानसून तंत्र एवं जलवायु प्रदेश",
      "अपवाह तंत्र, झीलें, सागर, बांध एवं जल संरक्षण तकनीकें",
      "प्राकृतिक वनस्पति",
      "वन्य जीव-जन्तु एवं अभयारण्य",
      "मृदाएं",
      "रबी एवं खरीफ की प्रमुख फसलें",
      "जनसंख्या — वृद्धि, घनत्व, साक्षरता एवं लिंगानुपात",
      "प्रमुख जनजातियाँ",
      "धात्विक एवं अधात्विक खनिज पदार्थ",
      "ऊर्जा संसाधन — परम्परागत एवं गैर-परम्परागत",
      "पर्यटन स्थल",
      "यातायात के साधन — राष्ट्रीय राजमार्ग, रेल एवं वायुयान",
    ],
  },
  {
    id: "polity",
    title: "राजस्थान पर विशेष बल के साथ भारतीय राजनीतिक व्यवस्था",
    points: [
      "भारतीय संविधान की प्रकृति — प्रस्तावना (उद्देशिका), मौलिक अधिकार, राज्य के नीति निर्देशक सिद्धान्त, मौलिक कर्तव्य, संघीय ढांचा, संवैधानिक संशोधन, आपातकालीन प्रावधान, जनहित याचिका",
      "संविधान सभा, भारतीय संविधान की विशेषताएं",
      "राष्ट्रपति, प्रधानमंत्री एवं मंत्रिपरिषद्, संसद, उच्चतम न्यायालय",
      "संघीय एवं राज्य कार्यपालिका, निर्वाचन आयोग, नियंत्रक एवं महालेखा परीक्षक, मुख्य सूचना आयुक्त, लोकपाल",
      "राष्ट्रीय मानवाधिकार आयोग, स्थानीय स्वायत्त शासन एवं पंचायती राज",
      "राजस्थान की राजनीतिक एवं प्रशासनिक व्यवस्था — राज्यपाल, मुख्यमंत्री, राज्य विधानसभा, उच्च न्यायालय, राजस्थान लोक सेवा आयोग, जिला प्रशासन, राज्य मानवाधिकार आयोग, लोकायुक्त, राज्य निर्वाचन आयोग, राज्य सूचना आयोग",
    ],
  },
  {
    id: "economy-india",
    title: "भारत की अर्थव्यवस्था",
    points: [
      "बजट निर्माण, बैंकिंग, लोक-वित्त, वस्तु एवं सेवा कर, राष्ट्रीय आय, संवृद्धि एवं विकास का आधारभूत ज्ञान",
      "राजकोषीय एवं मौद्रिक नीतियाँ",
      "सब्सिडी, लोक वितरण प्रणाली",
      "ई-कॉमर्स",
      "अर्थव्यवस्था के प्रमुख क्षेत्र — कृषि, उद्योग, सेवा एवं व्यापार क्षेत्रों की वर्तमान स्थिति, मुद्दे एवं पहल",
      "हरित क्रान्ति, श्वेत क्रान्ति एवं नीली क्रान्ति",
      "पंचवर्षीय योजनाएं एवं नियोजन प्रणाली",
      "प्रमुख आर्थिक समस्याएं एवं सरकार की पहल, आर्थिक सुधार एवं उदारीकरण",
    ],
  },
  {
    id: "economy-rajasthan",
    title: "राजस्थान की अर्थव्यवस्था",
    points: [
      "राजस्थान की खाद्य व व्यावसायिक फसलें, कृषि आधारित उद्योग",
      "वृहद सिंचाई एवं नदी घाटी परियोजनायें, बंजड़ भूमि व सूखा क्षेत्र विकास परियोजनायें, इन्दिरा गांधी नहर परियोजना",
      "उद्योगों का विकास व उनका स्थान — कृषि आधारित उद्योग, खनिज आधारित उद्योग, लघु कुटीर एवं ग्रामोद्योग",
      "निर्यात सामग्री, राजस्थानी हस्तकला",
      "गरीबी एवं बेरोजगारी — अवधारणा, प्रकार, कारण, निदान एवं वर्तमान फ्लैगशिप योजनाएं, सामाजिक न्याय एवं अधिकारिता — कमजोर वर्गों के लिए प्रावधान",
      "विभिन्न कल्याणकारी योजनायें, महात्मा गांधी राष्ट्रीय ग्रामीण रोजगार गारंटी अधिनियम (MNREGA), विकास संस्थायें, सहकारी आन्दोलन, लघु उद्यम एवं वित्तीय संस्थायें, संविधान के 73वें संशोधन के अनुरूप पंचायती राज संस्थाओं की ग्रामीण विकास में भूमिका",
    ],
  },
  {
    id: "science-tech",
    title: "विज्ञान एवं प्रौद्योगिकी",
    points: [
      "सूचना एवं संचार प्रौद्योगिकी",
      "रक्षा प्रौद्योगिकी, अंतरिक्ष प्रौद्योगिकी एवं उपग्रह",
      "विद्युत धारा, ऊष्मा, कार्य एवं ऊर्जा",
      "आहार एवं पोषण, रक्त समूह एवं RH कारक",
      "स्वास्थ्य देखभाल, संक्रामक, असंक्रामक एवं पशुजन्य रोग",
      "पर्यावरणीय एवं पारिस्थितिकीय परिवर्तन एवं इनके प्रभाव",
      "जैव-विविधता, प्राकृतिक संसाधनों का संरक्षण एवं संधारणीय विकास",
      "जन्तुओं एवं पादपों का आर्थिक महत्व",
      "कृषि-विज्ञान, उद्यान-विज्ञान, वानिकी एवं पशुपालन — राजस्थान के विशेष संदर्भ में",
      "विज्ञान एवं प्रौद्योगिकी विकास — राजस्थान के विशेष संदर्भ में",
      "भौतिक एवं रासायनिक परिवर्तन",
      "अम्ल, क्षार एवं लवण, ब्लीचिंग पाउडर, खाने का सोडा, प्लास्टर ऑफ पेरिस, साबुन एवं अपमार्जक",
    ],
  },
  {
    id: "reasoning",
    title: "तार्किक विवेचन एवं मानसिक योग्यता",
    points: [
      "Making series / analogy",
      "Figure matrix questions, Classification",
      "Alphabet test",
      "Passage and conclusions",
      "Blood relations",
      "Coding-decoding",
      "Direction sense test",
      "Sitting arrangement",
      "Input output",
      "Number Ranking and Time Square",
      "Making judgments",
      "Logical arrangement of words",
      "Inserting the missing character / number",
      "Mathematical operations, average, ratio",
      "Area of Triangle, Circle, Ellipse, Trapezium, Rectangle, Sphere, Cylinder",
      "Percentage",
      "Simple and compound interest",
      "Unitary Method",
      "Profit & Loss",
      "Average",
      "Ratio & Proportion",
      "Volume of sphere, cylinder, cube, cone",
    ],
  },
  {
    id: "hindi",
    title: "सामान्य हिन्दी (Language Knowledge)",
    points: [
      "सन्धि और सन्धि विच्छेद",
      "संज्ञा, सर्वनाम, विशेषण, क्रिया, क्रियाविशेषण, कारक, अव्यय",
      "समास, भेद, सामासिक पदों की रचना व विग्रह",
      "उपसर्ग एवं प्रत्यय",
      "विलोम शब्द, पर्यायवाची एवं अनेकार्थक शब्द",
      "विराम चिह्न",
      "ध्वनि एवं उसका वर्गीकरण",
      "पारिभाषिक शब्दावली (अंग्रेजी भाषा के पारिभाषिक शब्दों के समानार्थक शब्द)",
      "शब्द शुद्धि (अशुद्ध शब्दों का शुद्धिकरण)",
      "वाक्य शुद्धि (अशुद्ध वाक्यांश का शुद्धिकरण)",
      "मुहावरे एवं लोकोक्तियाँ",
      "राजभाषा हिन्दी — पत्र एवं उसके प्रकार, संवैधानिक स्थिति, कार्यालयी पत्र के प्रारूप के विशेष सन्दर्भ में",
    ],
  },
  {
    id: "english",
    title: "General English (Language Knowledge)",
    points: [
      "Use of Articles and Determiners",
      "Tense / sequence of Tenses",
      "Voice: Active and Passive",
      "Narration: Direct and Indirect",
      "Use of Prepositions",
      "Translation of ordinary/common English sentences into Hindi and vice-versa",
      "Synonyms and Antonyms",
      "Comprehension of a given passage",
      "Glossary of official, technical terms (with their Hindi version)",
      "Letter writing: Official, Demi-official, Circulars and Notices — questions will also be objective regarding the structure of a letter",
      "Idioms and Phrases",
      "One Word Substitution",
    ],
  },
  {
    id: "computer",
    title: "कम्प्यूटर का ज्ञान",
    points: [
      "Characteristics of Computers",
      "Computer Organization including RAM, ROM, File System, Input Devices",
      "Computer Software — Relationship between Hardware & Software",
      "Operating System",
      "MS-Office (Word, Excel/Spread Sheet, Power Point)",
    ],
  },
  {
    id: "current-affairs",
    title: "समसामयिक घटनाएं",
    points: [
      "राजस्थान, भारतीय एवं अन्तर्राष्ट्रीय महत्व की प्रमुख समसामयिक घटनाएं एवं मुद्दे",
      "वर्तमान में चर्चित व्यक्ति, स्थान एवं संस्थाएं",
      "खेल एवं खेलकूद संबंधी गतिविधियां",
    ],
  },
];
